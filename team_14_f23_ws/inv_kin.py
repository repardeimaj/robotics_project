...
# iterate while any error element is greater than its tolerance
	...
	# stop execution if the maximum number of iterations is exceeded
		...
		# compute the error
            dR = np.matmul(R , np.transpose(Rd))
            r1 = np.array(rox.R2rpy(dR))[None]
            r = np.zeros(np.shape(r1))
		...
		# compute the update
            ...
            JqJqT = np.matmul(Jq, np.transpose(Jq))
           
            j2 =np.matmul(np.matmul(np.transpose(Jq), np.linalg.inv(JqJqT + epsilon *np.identity(np.shape(JqJqT)[0]))),dX)
		# use the update to generate a new q
            q[:, iternum+1] = q[:, iternum] - np.transpose((alpha * j2))

            iternum = iternum + 1
        else:
            break
	# return the final estimate of q
   # print(rox.fwdkin(robot,q[:,iternum]))
    return q[:, iternum]
